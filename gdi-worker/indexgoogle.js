...
const authConfig = {
  ...
  "roots": [
    ...
    {
      "id": "1kuu3LQBiit-U-_98MAOBTOyrRMnVVA7C",
      "type": "folder",
      "name": "Drive Folder",
      "protect_file_link": false
    },
...
const crypto_base_key = "a3f8c2e1d947b605f2a8e3c1d4b7f920";
const hmac_base_key = "b4e9f2a1c836d705e2b9f4a3c1d8e705f3b2e9a4c837d106f4b3e2a9c1d8f705b4e9f2a1c836d705e2b9f4a3c1d8e705f3b2e9a4c837d106f4b3e2a9c1d8f705";
...
const uiConfig = {
  "theme": "darkly",
  "version": CDN_VERSION,
  "debug_mode": false, // ✅ Dimatikan untuk production
  "logo_image": false, // ✅ Pakai teks
  "logo_height": "",
  "logo_width": "100px",
  "favicon": 'https://cdn.jsdelivr.net/npm/@googledrive/index@' + CDN_VERSION + '/images/favicon.ico',
  "logo_link_name": "♛ ZAEINSTREAM VIP", // ✅ Nama logo teks
  "fixed_header": true,
  "header_padding": "80",
  "nav_link_1": "Home",
  ...
  "nav_link_4": "Contact",
  "fixed_footer": false,
  "hide_footer": false, // ✅ Footer ditampilkan
  ...
  "search_button_class": "btn btn-warning",
  "path_nav_alert_class": "alert alert-dark",
  "file_view_alert_class": "alert alert-warning",
  "file_count_alert_class": "alert alert-secondary",
  "contact_link": "https://wa.me/6282171872771", // ✅ WA Admin
  "copyright_year": new Date().getFullYear(),
  "company_name": "ZAEINSTREAM — Platform Streaming Eksklusif ❤️", // ✅ Footer keren
  "company_link": "https://wa.me/6282171872771",
  "credit": false, // ✅ Hapus credit pihak ketiga
  "display_size": true,
  "display_time": true,
  ...
  "poster": 'https://cdn.jsdelivr.net/npm/@googledrive/index@' + CDN_VERSION + '/images/poster.jpg',
  "audioposter": 'https://cdn.jsdelivr.net/npm/@googledrive/index@' + CDN_VERSION + '/images/music.jpg',
  "disable_audio_download": false,
  "render_head_md": true,
  "render_readme_md": true,
  "unauthorized_owner_link": "https://wa.me/6282171872771",
  "unauthorized_owner_email": "sholehhuddin21@gmail.com",
...
const asn_blocked = `<html>
  ...
  <body style="margin:0;padding:0;width:100%;height:100%;color:#b0bec5;display:table;font-weight:100;font-family:Lato">
  <div style="text-align:center;display:table-cell;vertical-align:middle">
  <div style="font-size:80px;margin-bottom:40px">Access Denied</div>
  </div></body></html>`;
...
async function handleRequest(request, event) {
  // ✅ CORS Preflight — wajib untuk cross-origin POST dari browser
  ...
  if (path == '/sw.js') {
    ...
    if (!swResp.ok) {
      return new Response('// service worker unavailable', { status: 200, headers: { 'Content-Type': 'application/javascript; charset=utf-8' } });
    }
    const swData = await swResp.text();
    return new Response(swData, { status: 200, headers: { 'Content-Type': 'application/javascript; charset=utf-8', 'Cache-Control': 'no-cache', 'Service-Worker-Allowed': '/' } });
  }
  if (path == '/app.js') {
    if (environment === 'production') {
      return new Response('Not found.', { status: 404 });
    }
    const js = await fetch('https://gitlab.com/GoogleDriveIndex/Google-Drive-Index/-/raw/dev/src/app.js', { method: 'GET' });
    if (!js.ok) {
      return new Response('// app.js fetch failed', { status: 200, headers: { 'Content-Type': 'application/javascript; charset=utf-8' } });
    }
    const data = await js.text();
    return new Response(data, { status: 200, headers: { 'Content-Type': 'application/javascript; charset=utf-8', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Credentials': 'true' } });
  }
  if (path == '/logout') {
  ...
  if (authConfig.enable_login) {
    ...
    if (path == '/download.aspx' && !authConfig.disable_anonymous_download) {
      ...
      if (response.ok) {
        ...
        if (login_database == 'kv') {
          ...
          for (let i = 0; i < authConfig.users_list.length; i++) {
            if (authConfig.users_list[i].username == username) { user_found = true; break; }
          }
        }
        if (!user_found) {
          const response = new Response('', {});
        ...
        const response2 = new Response('', { status: 302, headers: { 'Location': '/', 'Set-Cookie': `session=${encryptedSession}; path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${86400 * authConfig.login_days}` } });
        return response2;
      } else {
      ...
      if (login_database == 'kv') {
        ...
        if (kv_key !== null) { user_found = kv_key == password; }
      } else {
        for (let i = 0; i < authConfig.users_list.length; i++) {
          if (authConfig.users_list[i].username == username && authConfig.users_list[i].password == password) { user_found = true; break; }
        }
      }
      if (!user_found) {
        return new Response(JSON.stringify({ ok: false, message: 'Invalid username or password.' }), { status: 200, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
      }
      const current_time = Date.now();
      const session_time = current_time + 86400000 * authConfig.login_days;
      ...
      if (cookie && cookie.includes('session=')) {
        ...
        let username;
        try { username = await decryptString(session.split('|')[0]); } catch (_) {
        ...
        let session_time;
        try { session_time = await decryptString(session.split('|')[2]); } catch (_) {
          if (request.method === 'POST') { return new Response(JSON.stringify({ ok: false, message: 'Invalid session. Please log in again.' }), { status: 401, headers: { 'Content-Type': 'application/json; charset=utf-8' } }); }
          return login();
        }
        const current_time = Date.now();
  ...
  if (region && blocked_region.includes(region.toUpperCase())) {
    ...
    if (raw_id) {
      ...
      for (let i = 0; i < gds.length; i++) {
        try {
          ...
          if (result && result[0]) { found_path = result[0]; found_drive = (result[1] !== undefined) ? result[1] : i; break; }
        } catch (_) {}
      }
      if (found_path) {
    ...
    for (let i = 0; i < gds.length; i++) {
      try {
        ...
        if (result && result[0]) { found_path = result[0]; found_drive_idx = (result[1] !== undefined) ? result[1] : i; break; }
      } catch (_) {}
    }
    if (found_path) {
  ...
  if (path == '/copy' && request.method === 'POST') {
    ...
    let file_id;
    try { file_id = await decryptString(encrypted_id); } catch (_) { return new Response(JSON.stringify({ error: { message: 'Invalid file ID' } }), { status: 400, headers: { 'Content-Type': 'application/json;charset=UTF-8' } }); }
  ...
  if (authConfig['direct_link_protection']) {
    ...
    else if (referer.includes(hostname)) { console.log("Refer Detected"); }
  ...
  if (match) {
    ...
    if (command === 'search') {
      ...
      let id;
      try { id = await decryptString(formdata.id); } catch (_) { return new Response(JSON.stringify({ error: { message: 'Invalid ID' } }), { status: 400, headers: { 'Content-Type': 'application/json;charset=UTF-8' } }); }
...
async function fetchAccessToken() {
  ...
  const requestOption = { 'method': 'POST', 'headers': headers, 'body': enQuery(post_data) };
  let response;
  for (let i = 0; i < 3; i++) {
...
async function apiRequest(request, gd, user_ip) {
  ...
  if (path.slice(-1) == '/') {
    ...
    return new Response(JSON.stringify({ ...fileWithoutId, id: encryptedId, driveId: encryptedDriveId, link: link }), { status: 200, headers: { 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json;charset=UTF-8' } });
  }
}
...
async function handleId2Path(request, gd) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ message: isBadInput ? 'Invalid encrypted ID' : 'Request Failed or Path Not Found', error: String(error) }), { status: isBadInput ? 400 : 500, headers: { "content-type": "application/json", "Access-Control-Allow-Origin": authConfig.cors_domain } });
  }
}
...
async function findId2Path(gd, url) {
  ...
  try {
    const result = await gd.findPathById(id);
    if (result && result[0]) { found_path = result[0]; found_drive_idx = (result[1] !== undefined) ? result[1] : gd.order; }
  } catch (_) {}
  if (!found_path) {
    for (let i = 0; i < gds.length; i++) {
...
class googleDrive {
  ...
  async searchFilesinDrive(origin_keyword, page_token = null, page_index = 0) {
    ...
    if (is_share_drive) {
      ...
      params.includeItemsFromAllDrives = true; params.supportsAllDrives = true;
    }
    if (page_token) { params.pageToken = page_token; }
  ...
  async findPathById(child_id) {
    ...
    try { [p_files, drive_index_no] = await this.findParentFilesRecursion(child_id); } catch (error) { return null; }
  ...
  async findPathId(path) {
    ...
    for (const name of arr) {
      c_path += name + '/';
      if (typeof this.paths[c_path] == 'undefined') { const id = await this._findDirId(c_id, name); this.paths[c_path] = id; }
  ...
  async _getAccessToken() {
    const c = this.root.client_id && this.root.client_secret && this.root.refresh_token ? this.root : (this.root.service_account && this.root.service_account_json ? this.root : null);
...
async function download(id, range = '', inline, exportFmt = null) {
  ...
  if (exportEntry) {
    ...
    if (!res.ok) { const details = await res.text(); return new Response(details, { status: res.status, headers: { "content-type": "text/plain;charset=UTF-8" } }); }
...
addEventListener('fetch', event => {
  event.respondWith(
    handleRequest(event.request, event)
      .then(response => {
        // Allow this worker to be embedded in iframe from any origin
